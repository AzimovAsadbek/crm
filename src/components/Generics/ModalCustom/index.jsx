...
import GenericInput from "../Input/index.jsx";
...
import GenericModal from "../Modal/index.jsx";
...
const CustomModal = (props) => {
    ...
    const onSave = async () => {
        ...
        try {
            if (state.id) {
                await request(`/tabs/${props.tabname}/id/${state.id}`, {
                    method: "PUT",
                    body: state,
                });
            } else {
                await request(`/tabs/${props.tabname}`, {
                    method: "POST",
    ...
    return (
        <GenericModal {...props} onClose={onClose} onSave={onSave}>
            <Title type={"bold"} $font_size={18}>{props?.title}</Title>
