...
import { Body, ChildTitle, ChildWrapper, Container, Exit, Logo, Menu, MenuInfo, MenuTitle, Profile, Row, SidebarContainer, SubTitle, Title, Wrap, Wrapper } from "./style"
                        ...
                        sidebar.map(parent => {
                            ...
                            return !parent.hiddin && <div key={parent.id}>
                                <MenuInfo onClick={(e) => onOpen(parent, e)} $active={activePath ? "true" : ""}>
                                    ...
                                    {parent.children && <Row $active={open.includes(parent?.id) ? "true" : ""} />}
                                </MenuInfo>
                                {
                                    parent?.children?.map(child => {
                                        return <Wrap to={child.path} key={child.id}>
                                            <ChildWrapper $active={open.includes(parent?.id) ? "true" : ""} $activepath={location.pathname == child.path ? "true" : ""}>
                                                <ChildTitle>
                                                    <MenuTitle>
